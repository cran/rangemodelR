#' rangemod.1d - range cohesion model for ordered (and non spatial) data
...
#' @return If degen is FALSE, a data frame with two colums for mean and SD 
#'        of expected richness
#' 
#' \itemize{
...
#' reps <- 100
#' temp <- rangemod.1d(tempmat,nb = NULL,var = NULL,first = F,degen = F,
#'                    rsize = "observed",reps)
#' plot(temp[,1],ylim= c(min(temp[,1] -2),max(temp[,1]+2)),pch = 16)
#' 
#' segments(1:10,y0=temp[,1]-temp[,2],y1= temp[,1]+temp[,2])}
#' @export
rangemod.1d <- function(spmat,nb = NULL,var = NULL,first = F,degen = F,
  ...
  spmat <- spmat[,which(colSums(spmat) > 0)]
  #check rangesizes
  ...
  for(j in 1:reps){
    ...
    if(degen == T){degen.mats[[j]] <- mat.temp}
